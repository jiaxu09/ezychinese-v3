...
import { HanYuSentence } from '@/lib/types'
import {
  useAddHanYuSentence,
...
const NewSentences = ({ bookId, chapterId, lessonId }: NewSentencesProps) => {
  ...
  } = useAddHanYuSentence(`${bookId}-${chapterId}-${lessonId}`)

  const {
    mutate: deleteHanYuSentence,
    error: deleteError,
    isPending: deletePending
  } = useDeleteHanYuSentence(`${bookId}-${chapterId}-${lessonId}`)

  const { data: hanyu_sentences } = useQuery(
    useGetHanYuSentencesByChapter(`${bookId}-${chapterId}-${lessonId}`)
  )
