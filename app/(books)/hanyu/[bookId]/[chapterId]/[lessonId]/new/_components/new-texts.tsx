...
const NewTexts = ({ bookId, chapterId, lessonId }: NewTextsProps) => {
  ...
  } = useAddHanYuText(`${bookId}-${chapterId}-${lessonId}`)

  const {
    mutate: deleteHanYuText,
    error: deleteError,
    isPending: deletePending
  } = useDeleteHanYuText(`${bookId}-${chapterId}-${lessonId}`)

  const { data: hanyu_texts } = useQuery(
    useGetHanYuTextsByChapter(`${bookId}-${chapterId}-${lessonId}`)
  )
