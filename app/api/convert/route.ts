...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await fs.readFile(jsonDirectory + '/hsk6.json', 'utf8')

    if (!data) {
      return new NextResponse('hsk1 file not found', {
        status: 404,
      })
    }

    const hanzi = JSON.parse(data)
    const result = hanzi.map((item: any, index: any) => ({
      "No": item.No,
